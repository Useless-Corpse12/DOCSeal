...
) : IRequestHandler<DirectInviteCommand, string>
{
    public async Task<string> Handle(DirectInviteCommand cmd, CancellationToken cnt)
    {
        ...
        if (!isOwner) throw new Exception("Только владелец может приглашать");
        ...
        var body = $"Вас приглашают в организацию \"{org.Name}\" " +
                   $"на роль \"{cmd.Role}\".\n\nПерейдите по ссылке: {inviteLink}\n\n" +
                   $"Ссылка действительна {(cmd.DurationDays > 0 ? $"{cmd.DurationDays} " +
                                                                   $"дней" : "бессрочно")}.";
